...
TwitchOAuth2Client.prototype.getStreams = async function(user_logins, accessToken = this.access_token, tokenType = 'Bearer') {
  const queryParameters = user_logins instanceof Array ? user_logins.map((user_login) => uriEncode({ user_login })).join('&') : uriEncode({ user_login: user_logins });
  const response = await fetch(
    `${this.twitchAPIBaseUri}/streams?${queryParameters}`,
    {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
